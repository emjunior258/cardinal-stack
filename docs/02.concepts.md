...
## Application Map
A definition of all the Screens/Activities that are part of a USSD application with indication of context values required to fulfill each of them. The Application map is served by the Application backend.

The definition of context values required to fulfill an Activity is just an array of strings, which are the names of those values. May include names of session value keys and or names of provided context values:
...
* `user.location` - The location of the user (LAC, CID, MCC, MNC)
...
## Input Traps
It's essentially a Map of user inputs to Activity ids. An input trap tells the user which Activity to navigate to depending on the user input.
