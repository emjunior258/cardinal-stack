...
An application is what we get as a result of setting up **Backends** and **Entry points**. Application is basically a group/collection of Entry-points and Backends related to the same service/domain.

## Backend
A backend service that implements USSD application logic. An HTTP URL is specified to reach the backend.

A backend selects Key pair to be used by the Platform to Authenticate on it.
...
An entry-point may choose to disable caching.

An entry-point may or may not be allowed access to user location data.
...
## Activity Map
A definition of all the Screens/Activities that are part of a USSD application with indication of context values required to fulfill each of them and the caching configurations for each. The Activity map is served by the Application backend.
